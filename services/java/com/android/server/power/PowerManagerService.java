...
public final class PowerManagerService extends IPowerManager.Stub
    ...
    public void systemReady(TwilightService twilight, DreamManagerService dreamManager) {
        synchronized (mLock) {
            ...
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.WAKELOCK_BLOCKING_ENABLED),
                    false, mSettingsObserver, UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.WAKELOCK_BLOCKING_LIST),
                    false, mSettingsObserver, UserHandle.USER_ALL);
    ...
    private void updateSettingsLocked() {
        ...
        mWakeLockBlockingEnabled = Settings.XOPLAX.getIntForUser(resolver,
                Settings.XOPLAX.WAKELOCK_BLOCKING_ENABLED,
                0, UserHandle.USER_CURRENT);

        String blockedWakelockList = Settings.XOPLAX.getStringForUser(resolver,
                Settings.XOPLAX.WAKELOCK_BLOCKING_LIST,
                UserHandle.USER_CURRENT);
