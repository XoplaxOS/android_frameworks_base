...
public class AppTransition implements Dump {
    ...
    private class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(
                    Settings.XOPLAX.getUriFor(Settings.XOPLAX.ANIMATION_CONTROLS_DURATION),
                    false, this);
            resolver.registerContentObserver(
                    Settings.XOPLAX.getUriFor(Settings.XOPLAX.ANIMATION_CONTROLS_NO_OVERRIDE),
                    false, this);
            for (int i = 0; i < 10; i++) {
              resolver.registerContentObserver(
                    Settings.XOPLAX.getUriFor(Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[i]),
                    false, this);
            }
    ...
    private void updateSettings() {
        ContentResolver resolver = mContext.getContentResolver();
        for (int i = 0; i < 10; i++) {
            mActivityAnimations[i] = Settings.XOPLAX.getInt(resolver,
                 Settings.XOPLAX.ACTIVITY_ANIMATION_CONTROLS[i], 0);
        }

        mNoOverrides = Settings.XOPLAX.getInt(resolver,
                 Settings.XOPLAX.ANIMATION_CONTROLS_NO_OVERRIDE, 0) == 1;

        int temp = Settings.XOPLAX.getInt(resolver,
                 Settings.XOPLAX.ANIMATION_CONTROLS_DURATION, 0);
