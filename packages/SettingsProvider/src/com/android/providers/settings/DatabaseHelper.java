...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String TABLE_XOPLAX = "xoplax";

    static {
        mValidTables.add(TABLE_SYSTEM);
        mValidTables.add(TABLE_SECURE);
        mValidTables.add(TABLE_GLOBAL);
        mValidTables.add(TABLE_XOPLAX);
    ...
    private void createXoplaXTable(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS xoplax (" +
                "_id INTEGER PRIMARY KEY AUTOINCREMENT," +
                "name TEXT UNIQUE ON CONFLICT REPLACE," +
                "value TEXT" +
                ");");
        db.execSQL("CREATE INDEX IF NOT EXISTS xoplaxIndex1 ON xoplax (name);");
    }
    ...
    private void wipeDB(SQLiteDatabase db, int oldVersion, int upgradeVersion,
     int currentVersion) {
        ...
        db.execSQL("DROP TABLE IF EXISTS xoplax");
        db.execSQL("DROP INDEX IF EXISTS xoplaxIndex1");
        onCreate(db);
    ...
    private void loadXoplaXSettings(SQLiteDatabase db) {
        SQLiteStatement stmt = null;
        try {
            stmt = db.compileStatement("INSERT OR IGNORE INTO xoplax(name,value)"
                    + " VALUES(?,?);");
        } finally {
            if (stmt != null) stmt.close();
        }
