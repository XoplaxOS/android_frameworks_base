...
public class PieSysInfo extends PieSliceContainer implements ValueAnimator.AnimatorUpdateListener {
    ...
    private void setColor() {
        int textColor = Settings.XOPLAX.getIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_TEXT_COLOR, -2,
    ...
    public void draw(Canvas canvas, EdgeGesturePosition position) {
        ...
        if (mStaleData || Settings.XOPLAX.getIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_SHOW_TEXT, 1, UserHandle.USER_CURRENT) == 0) {
            return;
        }
