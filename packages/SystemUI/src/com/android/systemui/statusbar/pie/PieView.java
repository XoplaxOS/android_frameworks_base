...
public class PieView extends View implements View.OnTouchListener {
    ...
    private void getDimensions() {
        mPieScale = Settings.XOPLAX.getFloatForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_SIZE, PIE_CONTROL_SIZE_DEFAULT,
                UserHandle.USER_CURRENT);
        mMirrorRightPie = Settings.XOPLAX.getIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_MIRROR_RIGHT, 1,
    ...
    private void getColors() {
        ...
        int snapPaintColor = Settings.XOPLAX.getIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_SNAP_COLOR, -2,
                UserHandle.USER_CURRENT);
        if (snapPaintColor == -2) {
        ...
        float backgroundAlpha = Settings.XOPLAX.getFloatForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_BACKGROUND_ALPHA, 0.3f,
                UserHandle.USER_CURRENT);

        mBackgroundPaintColor = Settings.XOPLAX.getIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_BACKGROUND_COLOR, -2,
                UserHandle.USER_CURRENT);
    ...
    private void setupSnapPoints(int width, int height) {
        if (Settings.XOPLAX.getIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_SHOW_SNAP, 1,
    ...
    public void activate(Point center, EdgeGesturePosition position) {
        ...
        mShowBackground = Settings.XOPLAX.getIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_SHOW_BACKGROUND, 1,
                UserHandle.USER_CURRENT) == 1;
        getDimensions();
        getColors();
