...
import com.android.internal.util.xoplax.AwesomeConstants.AwesomeConstant;
...
public class NavigationBarView extends LinearLayout implements BaseStatusBar.NavigationBarCallback {
    ...
    public NavigationBarView(Context context, AttributeSet attrs) {
        ...
        mUserButtons = Settings.XOPLAX.getString(cr, Settings.XOPLAX.NAVIGATION_BAR_BUTTONS);
        mNavigationBarForceMenu = Settings.XOPLAX.getBoolean(cr, Settings.XOPLAX.NAVIGATION_MENU_FORCE, false);
        mNavigationBarMenuLocation = Settings.XOPLAX.getInt(cr, Settings.XOPLAX.NAVIGATION_MENU, 0);
        mShowDpadKeys = Settings.XOPLAX.getBoolean(cr, Settings.XOPLAX.NAVIGATION_BAR_DPAD_KEYS, false);
    ...
    public void setDisabledFlags(int disabledFlags, boolean force) {
        ...
        final boolean showNotifs = showSearch &&
                Settings.XOPLAX.getInt(mContext.getContentResolver(),
                        Settings.XOPLAX.LOCKSCREEN_NOTIFICATIONS, 1) == 1 &&
                Settings.XOPLAX.getInt(mContext.getContentResolver(),
                        Settings.XOPLAX.LOCKSCREEN_NOTIFICATIONS_PRIVACY_MODE, 0) == 0;
        ...
        if (mSettingsObserver == null) {
            mSettingsObserver = new ContentObserver(new Handler()) {
                ...
                public void onChange(boolean selfChange) {
                    mUserButtons = Settings.XOPLAX.getString(r, Settings.XOPLAX.NAVIGATION_BAR_BUTTONS);
                    mNavigationBarForceMenu = Settings.XOPLAX.getBoolean(r, Settings.XOPLAX.NAVIGATION_MENU_FORCE, false);
                    mNavigationBarMenuLocation = Settings.XOPLAX.getInt(r, Settings.XOPLAX.NAVIGATION_MENU, 0);
                    mShowDpadKeys = Settings.XOPLAX.getBoolean(r, Settings.XOPLAX.NAVIGATION_BAR_DPAD_KEYS, false);
                    setupNavigationButtons();
                    setMenuVisibility(mShowMenu, true /* force */);
                }
            };

            r.registerContentObserver(Settings.XOPLAX.getUriFor(Settings.XOPLAX.NAVIGATION_BAR_BUTTONS),
                    false, mSettingsObserver);
            r.registerContentObserver(Settings.XOPLAX.getUriFor(Settings.XOPLAX.NAVIGATION_BAR_DPAD_KEYS),
                    false, mSettingsObserver);
            r.registerContentObserver(Settings.XOPLAX.getUriFor(Settings.XOPLAX.NAVIGATION_MENU),
                    false, mSettingsObserver);
            r.registerContentObserver(Settings.XOPLAX.getUriFor(Settings.XOPLAX.NAVIGATION_MENU_FORCE),
                    false, mSettingsObserver);
        }
    }
