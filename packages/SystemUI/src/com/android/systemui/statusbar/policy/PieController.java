...
import com.android.internal.util.xoplax.ButtonConfig;
import com.android.internal.util.xoplax.ButtonsConstants;
import com.android.internal.util.xoplax.ButtonsHelper;
import com.android.internal.util.xoplax.Converter;
import com.android.internal.util.xoplax.ImageHelper;
import com.android.internal.util.xoplax.SlimActions;
...
public class PieController implements BaseStatusBar.NavigationBarCallback, PieView.OnExitListener,
    ...
    private final class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_GRAVITY), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_BUTTONS_CONFIG), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_SIZE), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_BUTTON_COLOR), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_BUTTON_PRESSED_COLOR), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_BUTTON_LONG_PRESSED_COLOR), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_BUTTON_OUTLINE_COLOR), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_ICON_COLOR), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_ICON_COLOR_MODE), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_BUTTON_ALPHA), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_BUTTON_PRESSED_ALPHA), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_BUTTONS_CONFIG_SECOND_LAYER), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_MENU), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_IME_CONTROL), false, this,
                    UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_TRIGGER_SENSITIVITY), false, this,
                    UserHandle.USER_ALL);
        }
        ...
        public void onChange(boolean selfChange, Uri uri) {
            ...
            if (uri.equals(Settings.XOPLAX.getUriFor(
                    Settings.XOPLAX.PIE_BUTTONS_CONFIG_SECOND_LAYER))) {
    ...
    private void setupListener() {
        ...
        mPieTriggerSlots = Settings.XOPLAX.getIntForUser(resolver,
                Settings.XOPLAX.PIE_GRAVITY, EdgeGesturePosition.LEFT.FLAG,
                UserHandle.USER_CURRENT);

        mSensitivity = Settings.XOPLAX.getInt(resolver,
                Settings.XOPLAX.PIE_TRIGGER_SENSITIVITY, mSensitivity);
        ...
        if (Settings.XOPLAX.getIntForUser(resolver,
                Settings.XOPLAX.PIE_IME_CONTROL, 1,
    ...
    private void setupNavigationItems() {
        ...
        float diff = PieView.PIE_ICON_START_SIZE_FACTOR -
                Settings.XOPLAX.getFloatForUser(resolver,
                        Settings.XOPLAX.PIE_SIZE, PieView.PIE_CONTROL_SIZE_DEFAULT,
                        UserHandle.USER_CURRENT);
        if (diff > 0.0f) {
            mIconResize = true;
        ...
        mShowMenuVisibility = Settings.XOPLAX.getIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_MENU, MENU_VISIBILITY_SYSTEM,
                UserHandle.USER_CURRENT);
    ...
    private Drawable prepareBackIcon(Drawable d, boolean customIcon) {
        int customImageColorize = Settings.XOPLAX.getIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_ICON_COLOR_MODE, 0,
                UserHandle.USER_CURRENT);
        int drawableColor = Settings.XOPLAX.getIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_ICON_COLOR, -2, UserHandle.USER_CURRENT);
        if (drawableColor == -2) {
    ...
    public void onSnap(EdgeGesturePosition position) {
        ...
        int triggerSlots = Settings.XOPLAX.getIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_GRAVITY, EdgeGesturePosition.LEFT.FLAG,
                UserHandle.USER_CURRENT);

        triggerSlots = triggerSlots & ~mPosition.FLAG | position.FLAG;

        Settings.XOPLAX.putIntForUser(mContext.getContentResolver(),
                Settings.XOPLAX.PIE_GRAVITY, triggerSlots,
                UserHandle.USER_CURRENT);
    }
