...
public class ViewConfiguration {
    ...
    private ViewConfiguration(Context context) {
        ...
        if (Settings.XOPLAX.getInt(resolver,
                          Settings.XOPLAX.ANIMATION_CONTROLS_NO_SCROLL, 0) != 1) {
            ...
            SCROLL_FRICTION = Settings.XOPLAX.getFloat(resolver,
                          Settings.XOPLAX.CUSTOM_SCROLL_FRICTION, DEFAULT_SCROLL_FRICTION);
            int maximumFlingVelocity = Settings.XOPLAX.getInt(resolver,
                          Settings.XOPLAX.CUSTOM_FLING_VELOCITY, DEFAULT_MAXIMUM_FLING_VELOCITY);
            if (maximumFlingVelocity == 0) {
                MAXIMUM_FLING_VELOCITY = DEFAULT_MAXIMUM_FLING_VELOCITY;
            } else {
                MAXIMUM_FLING_VELOCITY = maximumFlingVelocity;
            }
            int overScrollDistance = Settings.XOPLAX.getInt(resolver,
                          Settings.XOPLAX.CUSTOM_OVERSCROLL_DISTANCE, DEFAULT_OVERSCROLL_DISTANCE);
            if (overScrollDistance > 100) {
                OVERSCROLL_DISTANCE = DEFAULT_OVERSCROLL_DISTANCE;
            } else {
                OVERSCROLL_DISTANCE = overScrollDistance;
            }
            int overFlingDistance = Settings.XOPLAX.getInt(resolver,
                          Settings.XOPLAX.CUSTOM_OVERFLING_DISTANCE, DEFAULT_OVERFLING_DISTANCE);
